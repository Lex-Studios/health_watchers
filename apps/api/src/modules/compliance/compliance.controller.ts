...
router.get('/baas', authenticate, requireRoles(['CLINIC_ADMIN', 'SUPER_ADMIN']), async (req: Request, res: Response) => {
  try {
...
router.post('/baas', authenticate, requireRoles(['CLINIC_ADMIN', 'SUPER_ADMIN']), async (req: Request, res: Response) => {
  try {
    const { businessAssociate, status, signedDate, expiryDate, documentUrl, notes } = req.body;

    if (!businessAssociate) {
      return res.status(400).json({ error: 'ValidationError', message: 'businessAssociate is required' });
    }
...
router.get('/breaches', authenticate, requireRoles(['CLINIC_ADMIN', 'SUPER_ADMIN']), async (req: Request, res: Response) => {
  try {
    const breaches = await BreachNotificationModel.find({ clinicId: req.user!.clinicId }).sort({ detectedAt: -1 });
    res.json(breaches);
  } catch (err) {
...
router.post('/breaches', authenticate, requireRoles(['CLINIC_ADMIN', 'SUPER_ADMIN']), async (req: Request, res: Response) => {
  try {
    const { breachType, description, affectedRecords } = req.body;

    if (!breachType || !description || !affectedRecords) {
      return res.status(400).json({ error: 'ValidationError', message: 'breachType, description, and affectedRecords are required' });
    }
